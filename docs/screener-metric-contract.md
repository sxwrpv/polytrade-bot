...
The standalone public screener preserves unavailable metrics rather than coercing them to zero. The authenticated followed-wallet API still maps a missing `consistency_score` to the legacy bronze tier; that tier is not shown as a public screener metric.
...
- **Sort/filter:** neither is exposed by the public screener.
...
## Daily PnL and freshness metadata
...
- **Sort/filter:** the public screener supports the completeness toggle as a minimum threshold; sorting is not supported. Disabling partial-history inclusion applies `history_days >= selected period`, so null and shorter-coverage rows are excluded. This only checks fetched TRADE history and does not certify REDEEM or positions completeness.
...
## Copy Score — a third party's metric, carried alongside

Copy Score is **not** a PolyTrade metric and is deliberately absent from every entry above. `backend.core.trader_stats.SCREENER_METRIC_CONTRACT` does not define it, `/api/public/screener/*` does not publish it, and `tests/test_public_screener.py::test_no_opaque_composite_score_is_published` keeps it that way. PolyTrade publishes no composite score of its own because its inputs are partial by construction and a single number would conceal that.

The screener nevertheless *shows* one, because ranking wallets by what survives being copied is the question a copy-trading product's readers actually have. It is handled as a clearly separate object:

- **Owner and source.** Polycopy's public discover dataset (`https://polycopy.app/api/v2/discover/dataset`), covering roughly 3,700 wallets. Computing it needs a full-chain indexer PolyTrade does not run.
- **Transport.** A static, committed asset at `frontend/public/screener-cohort.json`, generated by `scripts/build_cohort.mjs`. It never travels on a PolyTrade API response and is never joined into `trader_cache`.
- **Attribution.** Every board that shows a score names the owner and prints the snapshot's generation date. Past two days the notice says outright that the snapshot describes a board that has already moved.
- **Coverage.** The cohort is a subset of PolyTrade's cache. A wallet it does not cover is rendered **Not scored** — the absence of a score, not a low one — and the board states how many of the rows on screen are scored.

### The two boards

The surface never blends the two sources into one table.

| Ordering | Rows come from | Money columns are | Copy Score is |
|---|---|---|---|
| PnL, Win rate, Volume | PolyTrade's live cache, server-ranked and paged | PolyTrade's, live | an overlay, where the cohort covers the wallet |
| Copy Score, ROI | the cohort snapshot, ranked in the browser | the snapshot's, dated | the ranking key |

Copy Score and ROI are ordered client-side because the API ranks by `pnl`, `winrate` and `volume` only, and a figure the cache does not hold cannot be ordered a page at a time. `buildPublicQuery` still rejects both — the API's sort vocabulary is unchanged.

### Window vocabulary

The cohort's windows are 7D, 30D and **lifetime**, which is not the live board's 90D. Selecting 90D and then a cohort ordering shows the lifetime board labelled `ALL`; it is never relabelled 90D, because that would put one window's figures under another window's heading.
...
Constants recovered from Polycopy's shipped client are marked `VERBATIM` in `frontend/src/screener/cohortModel.js`; constants fitted from its published dataset are marked `INFERRED` and are stated as estimates wherever they reach the screen. The distinction is deliberate — keep it when changing them.
